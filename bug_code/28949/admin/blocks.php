...
/// Print the table of all blocks
    ...
    foreach ($blocks as $blockid=>$block) {
        ...
        $delete = '<a href="blocks.php?delete='.$blockid.'&amp;sesskey='.sesskey().'&amp;_racetrkrnonce='.$_REQUEST['_racetrkrnonce'].'">'.$strdelete.'</a>';
        ...
        if ($count>0) {
            $blocklist = "<a href=\"{$CFG->wwwroot}/course/search.php?blocklist=$blockid&amp;sesskey=".sesskey().'&amp;_racetrkrnonce='.$_REQUEST['_racetrkrnonce']."\" ";
        ...
        if (!$blockobject) {
            ...
            $visible = '<a href="blocks.php?hide='.$blockid.'&amp;sesskey='.sesskey().'&amp;_racetrkrnonce='.$_REQUEST['_racetrkrnonce'].'" title="'.$strhide.'">'.
                       '<img src="'.$OUTPUT->pix_url('i/hide') . '" class="icon" alt="'.$strhide.'" /></a>';
        } else {
            $visible = '<a href="blocks.php?show='.$blockid.'&amp;sesskey='.sesskey().'&amp;_racetrkrnonce='.$_REQUEST['_racetrkrnonce'].'" title="'.$strshow.'">'.
        ...
        if (!$blockobject) {
            ...
            $undeletable = '<a href="blocks.php?unprotect='.$blockid.'&amp;sesskey='.sesskey().'&amp;_racetrkrnonce='.$_REQUEST['_racetrkrnonce'].'" title="'.$strunprotect.'">'.
                       '<img src="'.$OUTPUT->pix_url('t/unlock') . '" class="icon" alt="'.$strunprotect.'" /></a>';
        } else {
            $undeletable = '<a href="blocks.php?protect='.$blockid.'&amp;sesskey='.sesskey().'&amp;_racetrkrnonce='.$_REQUEST['_racetrkrnonce'].'" title="'.$strprotect.'">'.
    ...
    if (!empty($incompatible)) {
        ...
        foreach ($incompatible as $block) {
            $table->add_data(array(
                $block->name,
                '<a href="blocks.php?delete='.$block->id.'&amp;sesskey='.sesskey().'&amp;_racetrkrnonce='.$_REQUEST['_racetrkrnonce'].'">'.$strdelete.'</a>',
            ));
        }
        $table->print_html();
    }
